#===============================================================================
#================== Gibbs Sampling for Rider Pickup Locations ==================
#===============================================================================
...
#' Bayesian model that assumes the rider pickup locations are distributed from a 
#' multivariate normal distribution (where the mean vector and covariance matrix 
...
# Read the data for rider pickup locations
data <- read.csv("rider_ploc.csv")
...
inits <- list(list(mu=c(0, 0), Omega=matrix(c(0.01,0,0,0.01), nrow=2, ncol=2)), 
              list(mu=c(10, 10), Omega=matrix(c(0.05,0,0,0.05), nrow=2, ncol=2)), 
              list(mu=c(7.5, 12.5), Omega=matrix(c(0.1,0,0,0.1), nrow=2, ncol=2)))
...
png(filename="traceplots_pickup_sigma.png", width=2000, height=2000, res=300)
plot(samp[, 1:3])
dev.off()

png(filename="traceplots_pickup_mu.png", width=2000, height=1600, res=300)
plot(samp[, 4:5])
dev.off()
